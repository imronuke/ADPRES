...
WRITE(*, *) "--------------------------------------------------------------"
WRITE(*,'(I4, F10.3, F10.4, ES15.4, 12F9.2)') 0, 0., 0., &
1.0, (bpos(n), n = 1, nb)
...
WRITE(ounit, *) "-------------------------------------------"
WRITE(ounit,'(I4, F10.3, F10.4, ES15.4)') 0, 0., 0., 1.0
...
DO i = 1, imax
    ...
    WRITE(*,'(I4, F10.3, F10.4, ES15.4, 12F9.2)') step, t2, rho/tbeta, &
    tpow2/tpow1, (bpos(n), n = 1, nb)

    IF (maxi) THEN
        WRITE(ounit,'(I4, F10.3, F10.4, ES15.4, A35)') step, t2, rho/tbeta, &
        tpow2/tpow1, 'OUTER ITERATION DID NOT CONVERGE'
    ELSE
        WRITE(ounit,'(I4, F10.3, F10.4, ES15.4)') step, t2, rho/tbeta, &
        tpow2/tpow1
    END IF
...
USE sdata, ONLY: ng, nnod, sigr, nf, &
                 ...
                 ft, ftx1, fty1, ftz1, ftx2, fty2, ftz2, &
                 aprad, apaxi, afrad, m, mat
USE InpOutp, ONLY: XS_updt, XStab_updt, ounit, AsmPow, AxiPow, AsmFlux, bxtab
...
DO i = 1, imax
  ...
  ! Calculate xsec after pertubation
   ...
   IF (bxtab == 0) THEN
     DO g = 1, ng
        DO n = 1, nnod
           sigr(n,g) = sigr(n,g) + 1._DP / (velo(g) * tstep1) + omeg(n,g) / velo(g)
        END DO
     END DO
   ELSE
     DO g = 1, ng
        DO n = 1, nnod
           sigr(n,g) = sigr(n,g) + &
           1._DP / (m(mat(n))%velo(g) * tstep1) + omeg(n,g) / m(mat(n))%velo(g)
        END DO
     END DO
   END IF
    ...
    IF (maxi) THEN
      WRITE(ounit,'(I4, F10.3, F10.4, ES15.4, 4F10.2, A35)') step, t2, rho/tbeta, &
...
DO i = 1, imax
  ...
  ! Modify removal xsec
    ...
    IF (bxtab == 0) THEN
      DO g = 1, ng
         DO n = 1, nnod
            sigr(n,g) = sigr(n,g) + 1._DP / (velo(g) * tstep2) + omeg(n,g) / velo(g)
         END DO
      END DO
    ELSE
      DO g = 1, ng
         DO n = 1, nnod
            sigr(n,g) = sigr(n,g) + &
            1._DP / (m(mat(n))%velo(g) * tstep2) + omeg(n,g) / m(mat(n))%velo(g)
         END DO
      END DO
    END IF
...
USE sdata, ONLY: ng, nnod, m, mat, chi, iBeta, nf, f0, nuf
...
c0, cx1, cy1, cz1, cx2, cy2, cz2, iBeta, lamb, m, mat, nuf, ng
  ...
  DO n = 1, nnod
     DO j = 1, nf
       IF (nuf(n,ng) > 0.) THEN  !If it is fuel
         ...
         c0(n,j)  = blamb * fs0(n)
         cx1(n,j) = blamb * fsx1(n)
         cy1(n,j) = blamb * fsy1(n)
         cz1(n,j) = blamb * fsz1(n)
         cx2(n,j) = blamb * fsx2(n)
         cy2(n,j) = blamb * fsy2(n)
         cz2(n,j) = blamb * fsz2(n)
       ELSE
         c0(n,j) = 0.
...
IF (bxtab == 1) THEN
  DO n = 1, nnod
     DO j = 1, nf
       IF (nuf(n,ng) > 0.) THEN  !If it is fuel
         lat = (1._DP + m(mat(n))%lamb(j) * h)
         beth = m(mat(n))%iBeta(j) * h
         c0(n,j)  = (c0(n,j)  + beth * fs0(n)) / lat
         cx1(n,j) = (cx1(n,j) + beth * fsx1(n)) / lat
         cy1(n,j) = (cy1(n,j) + beth * fsy1(n)) / lat
         cz1(n,j) = (cz1(n,j) + beth * fsz1(n)) / lat
