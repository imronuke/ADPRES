...
SUBROUTINE kinet_par(gm, dsigr, dnuf, dsigs, sf, xA, xrho)
...
USE sdata, ONLY: ng, nnod, chi, velo, nf, iBeta,tbeta, &
...
REAL, DIMENSION(nnod,ng) :: chib

chib = 0.
DO g = 1, ng
   DO n = 1, nnod
      DO i = 1, nf
         chib(n,g) = chib(n,g) + iBeta(i) * chi(n,g)
      END DO
   END DO
END DO
...
DO g = 1, ng
    DO n = 1, nnod
        vdum2(n) = vdum2(n) + ((1.  - tbeta) * chi(n,g) + chib(n,g)) * vdum(n) * af(n,g)
    END DO
END DO
...
DO g = 1, ng
  ...
  DO n = 1, nnod
     ...
     fiss = af(n,g) * ((1.  - tbeta) * chi(n,g) + chib(n,g)) * vdum2(n) * vdel(n)
...
CALL kinet_par(1.0, sigr, nuf, sigs, f0, A, rho)
...
WRITE(ounit, *) "--------------------------------------------------------------"
WRITE(ounit,'(I4, F10.3, F10.4, ES15.4, 12F9.2)') 0, 0., rho/ptbet, 1.0, (bpos(n), n = 1, nb)
...
DO i = 1, imax
    ...
    CALL kinet_par(gamma2, sigrp, nuf, sigs, ft, A, rho)
    ...
    WRITE(ounit,'(I4, F10.3, F10.4, ES15.4, 12F9.2)') step, t2, rho/ptbet, tpow2/tpow1, (bpos(n), n = 1, nb)
...
USE sdata, ONLY: ng, nnod, sigr, nuf, sigs, f0, &
                 iBeta, lamb, nf, nout, nac, &
                 ttot, tdiv, tstep1, tstep2, &
                 ix, iy, iz, zdel, vdel, Ke, npow, pow, ppow, node_nf, &
                 bcon, ftem, mtem, cden, &
                 fbpos, bpos, tmove, bspeed, mdir, &
                 nout, nac, nb, xnuf, dnuf, sigr, velo, &
                 f0, fx1, fy1, fz1, fx2, fy2, fz2, &
                 ...
                 ct, ctx1, cty1, ctz1, ctx2, cty2, ctz2
USE InpOutp, ONLY: XS_updt, bther, ounit
USE nodal, ONLY: nodal_coup4, outer4, outertr, powdis, powdis2
...
WRITE(ounit, *) " Step  Time(s)  React.($)   Rel. Power   CR Bank Pos. (1-end)"
WRITE(ounit, *) "--------------------------------------------------------------"
WRITE(ounit,'(I4, F10.3, F10.4, ES15.4, 12F9.2)') 0, 0., rho/ptbet, ppow*0.01, (bpos(n), n = 1, nb)
...
DO i = 1, imax
   ...
   WRITE(ounit,'(I4, F10.3, F10.4, ES15.4, 12F9.2)') step, t2, rho/ptbet, xppow, (bpos(n), n = 1, nb)
...
DO i = 1, imax
    ...
    END DO
    CALL nodal_coup4()
    CALL outertr(200, tstep2, ft, ftx1, fty1, ftz1, ftx2, fty2, ftz2)

    ! Predict precusor density
    CALL precusor(tstep2, ct, ctx1, cty1, ctz1, ctx2, cty2, ctz2)
...
DO i = 1, itot
    ...
    k1 = (xrho - ptbet) * xamp / xA + summ
    k2 = (xrho - ptbet) * (xamp + k1 * 0.5 * h) / xA + summ
    k3 = (xrho - ptbet) * (xamp + k2 * 0.5 * h) / xA + summ
    k4 = (xrho - ptbet) * (xamp + k3 * h) / xA + summ
