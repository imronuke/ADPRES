...
USE sdata, ONLY: vdel, nnod, iz, nzz
...
USE sdata, ONLY: nnod, mtem, cden, ftem, tin, cflow, nxx, nyy, cf, ent, heatf, &
                 tfm, nt, rpos, rdel, rf, rg, rc, farea, dia, pi, &
                 zdel, ix, iy, iz, nflow
...
REAL(DP) :: xa, xc
REAL(DP) :: fdens = 10.412D3            ! UO2 density (kg/m3)
REAL(DP) :: cldens = 6.504D3               ! Cladding density (kg/m3)
...
REAL(DP), DIMENSION(nxx, nyy) :: eflow  ! channel mass flow rate at boundary
...
DO n = 1, nnod
  ...
  zd     = zdel(iz(n)) * 0.01_DP                                                ! zdel in meter
  vol    = farea * zd                                                           ! channel node volume

  IF (iz(n) == 1) THEN                                                          ! Calculate coolant enthalpy
    eps = mdens * vol / h
    ent(n) = (cpline * zd + 2._DP * nflow(n) * enti + eps * entp(n)) &
             / (eps + 4._DP * nflow(n) - 2._DP * cflow)                         ! Calculate coolant enthalpy
    CALL gettd(ent(n), mtem(n), cden(n), Pr, kv, tcon, R)                       ! Get corresponding temp and density
    eps = -R / mdens
    nflow(n) = (cflow * (1._DP + eps * ent(n)) + 0.5_DP * eps * cpline * zd) &
             / (1._DP + eps * (2._DP * ent(n) - enti))                          ! Calc. local channel mass flow rate
    entm(ix(n),iy(n)) = 2._DP * ent(n) - enti                                   ! Calc. enthalpy at node boundary
    eflow(ix(n),iy(n)) = 2._DP * nflow(n) - cflow                               ! Calc. local mass flow rate at node boundary
  ELSE
    eps = mdens * vol / h
    ent(n) = (cpline * zd + 2._DP * nflow(n) * entm(ix(n),iy(n)) + eps * entp(n)) &
             / (eps + 4._DP * nflow(n) - 2._DP * eflow(ix(n),iy(n)))
    CALL gettd(ent(n), mtem(n), cden(n), Pr, kv, tcon, R)
    eps = -R / mdens
    nflow(n) = (eflow(ix(n),iy(n)) * (1._DP + eps * ent(n)) &
               + 0.5_DP * eps * cpline * zd) &
               / (1._DP + eps * (2._DP * ent(n) - entm(ix(n),iy(n))))
    entm(ix(n),iy(n)) = 2._DP * ent(n) - entm(ix(n),iy(n))
    eflow(ix(n),iy(n)) = 2._DP * nflow(n) - eflow(ix(n),iy(n))
  END IF

  hs = geths(cden(n), tcon, kv, Pr)                                             ! Calculate heat transfer coef
  ...
  DO i = 2, nt-2
    ...
    kt  = 2._DP * kt1 * kt2 / (kt1 + kt2)
    cp = getcpf(tfm(n,i))
    eta = fdens * cp * (rpos(i)**2 - rpos(i-1)**2) / (2._DP * h)
    xa = xc
    xc = kt * rpos(i) / rdel(i)
    a(i) = -xa
    b(i) =  xa + xc + eta
  ...
  eta = fdens * cp * (rf**2 - rpos(nt-2)**2) / (2._DP * h)
  xa = xc
  xc = rg * hg
  a(nt-1) = -xa
  b(nt-1) =  xa + xc + eta
  ...
  cp = getcpc(tfm(n,nt))
  eta = cldens * cp * (rpos(nt)**2 - rg**2) / (2._DP * h)
  xa = xc
  ...
  eta = cldens * cp * (rc**2 - rpos(nt)**2) / (2._DP * h)
  xa = xc
  xc = rc * hs
  a(nt+1) = -xa
  b(nt+1) =  xa + xc + eta
...
DO n = 1, nnod
  ...
  ! zd = zdel(iz(n)) * 0.01_DP
  ! IF (iz(n) == 1) THEN                                              ! For most bootom channel
  !   ent(n) = (cpline * zd + 2._DP * nflow(n) * enti) &
  !          / (4._DP * nflow(n) - 2._DP * cflow)                    ! Calculate coolant enthalpy
  !   CALL gettd(ent(n), mtem(n), cden(n), Pr, kv, tcon)             ! Get corresponding temp and density
  !   entm(ix(n),iy(n)) = 2._DP * ent(n) - enti                      ! Extrapolate enthalpy at node boundary
  !   eflow(ix(n),iy(n)) = 2._DP * nflow(n) - cflow
  !   WRITE(*,*) eflow(ix(n),iy(n))
  ! ELSE
  !   ent(n) = (cpline * zd + 2._DP * nflow(n) * entm(ix(n),iy(n))) &
  !          / (4._DP * nflow(n) - 2._DP * eflow(ix(n),iy(n)))       ! Calculate coolant enthalpy
  !   CALL gettd(ent(n), mtem(n), cden(n), Pr, kv, tcon)             ! Get corresponding temp and density
  !   entm(ix(n),iy(n)) = 2._DP * ent(n) - entm(ix(n),iy(n))         ! Extrapolate enthalpy at node boundary
  !   eflow(ix(n),iy(n)) = 2._DP * nflow(n) - eflow(ix(n),iy(n))
  ! END IF

  hs = geths(cden(n), tcon, kv, Pr)
  pdens = (1._DP - cf) * 100._DP * xpline(n) / (pi * rf**2)        ! Fuel pin Power Density (W/m3)
